// whatsapp.js
// Brynix – BOT de projeto com menu interativo e integrações (Sheets/Drive/IA)
...
async function handleNext(msg, link) {
  try {
    ...
    const title = `${B(`${link.projectName} — Próximas (hoje/amanhã)`)}\n`;
...
async function handleNote(msg, link, noteText) {
  ...
  try {
    // no futuro: salvar no LOG/Sheets
...
async function sendMenu(msg, link) {
  const project = link?.projectName ? `Projeto: *${link.projectName}*\n\n` : '';
  const body = `${project}Escolha um atalho:`;
...
function wireEvents(c) {
  ...
  c.on('message', async (msg) => {
    try {
      ...
      // 0) Resposta de BOTÕES (menu)
      if (msg.type === 'buttons_response') {
        const val = (msg.body || '').toLowerCase();
        const link = getProjectLink(chatId);
        if (!link) return sendMenu(msg, null);

        if (val.includes('resumo curto'))  return handleSummaryBrief(msg, link);
        if (val.includes('resumo'))        return handleSummaryComplete(msg, link);
        if (val.includes('próximas') || val.includes('proximas')) return handleNext(msg, link);
        if (val.includes('atrasadas'))     return handleLate(msg, link);
        if (val.includes('participantes')) return handleWho(msg, link);
        if (val.includes('silenciar'))     { muteMap.set(chatId, true);  return msg.reply(I('ok, fico em silêncio até /mute off')); }
        if (val.includes('voltar a falar')){ muteMap.delete(chatId);     return msg.reply(I('voltei a falar 😉')); }
      ...
      // 2) Se estiver mutado, sai (exceto unmute acima)
      ...
      // 4) Upload de mídia (salva em Drive do projeto)
      ...
      if (isCommand && /^\/setup/i.test(text)) {
        ...
        return sendMenu(msg, { projectName });
      }

      // /help e /menu → botões
      if (isCommand && /^\/(help|menu)$/i.test(text)) {
        const link = getProjectLink(chatId);
        await sendMenu(msg, link || { projectName: '' });
        return;
      }

      const link = getProjectLink(chatId);
      if (!link) {
        // se chamar “menu/ajuda” sem setup, mostra help textual
        if (/(menu|ajuda|help)/i.test(text)) return handleHelp(msg, null);
      ...
      switch (nlu.intent) {
        case INTENTS.HELP:
          await sendMenu(msg, link); return;
        ...
        case INTENTS.MUTE_ON:
          muteMap.set(chatId, true); return msg.reply(I('ok, fico em silêncio até /mute off'));
        case INTENTS.MUTE_OFF:
          muteMap.delete(chatId);   return msg.reply(I('voltei a falar 😉'));
        default:
          // Se foi mencionado e disse “menu/ajuda”, abre o menu
          if (mentioned && /\b(menu|ajuda|help)\b/i.test(text)) return sendMenu(msg, link);
          // fallback: help textual
